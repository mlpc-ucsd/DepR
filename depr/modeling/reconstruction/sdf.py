...
class SdfModel(nn.Module):
    def __init__(self, config_json):
        super().__init__()
        self.model = SdfDecoder(
            d_in=config_json["channels"],
            d_out=1,
        ...
        self.model.load_state_dict(
            torch.load(config_json["ckpt_path"], map_location="cpu")
        )
        self.model.eval()
    ...
    def forward_with_plane_features(self, plane_features, xyz):
        ...
        xyz: B, N, 3
        """
        point_features = self.get_points_plane_features(
            plane_features, xyz
    ...
    def forward(self, plane_features, xyz):
        """
        plane_features: B, 3, D, res, res (e.g. B, 3, 32, 64, 64)
        xyz: B, N, 3
        """
        point_features = self.get_points_plane_features(
            plane_features, xyz
